...
export abstract class Boss extends BaseEntity {
    ...
    constructor(
        ...
        this.sprite.setSize(48, 64);
        this.sprite.setOffset(24, 20);
    ...
    protected setupAnimations(texture: string) {
        const textures = [texture, 'boss_necromancer_phase1', 'boss_necromancer_phase2', 'boss_necromancer_phase3'];
        
        for (const tex of textures) {
            if (!this.scene.textures.exists(tex)) continue;

            if (!this.scene.anims.exists(`${tex}_idle`)) {
                this.scene.anims.create({
                    key: `${tex}_idle`,
                    frames: this.scene.anims.generateFrameNumbers(tex, { start: 0, end: 3 }),
                    frameRate: 6,
                    repeat: -1
                });
            }

            if (!this.scene.anims.exists(`${tex}_cast`)) {
                this.scene.anims.create({
                    key: `${tex}_cast`,
                    frames: this.scene.anims.generateFrameNumbers(tex, { start: 4, end: 7 }),
                    frameRate: 8,
                    repeat: 0
                });
            }

            if (!this.scene.anims.exists(`${tex}_death`)) {
                this.scene.anims.create({
                    key: `${tex}_death`,
                    frames: this.scene.anims.generateFrameNumbers(tex, { start: 8, end: 11 }),
                    frameRate: 6,
                    repeat: 0
                });
            }
